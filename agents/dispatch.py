...
def build_prompt(role: str, msg: dict, run_id: str, mailroom: Path) -> str:
    ...
    return (
        ...
        f"your commit at its tip, or the message is retained). The schema is "
        f"CLOSED (additionalProperties: false): a field it does not define — "
        f"issue, pr, pr_url, notes, checks_run — makes the whole result "
        f"INVALID and your finished work is retained as if it never "
        f"happened. Record provenance in the commit message or PR body, "
        f"never in the result file. Do NOT "
...
def _assess_anti_loop(mailroom: Path, worktree: Path, *, task_id: str,
                      ...
                      role: str | None = None, intent: str | None = None):
