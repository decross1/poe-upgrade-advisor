...
def load_cases(corpus: Path = CORPUS) -> tuple[dict, list[CorpusCase]]:
    ...
    # The legacy 15-build manifest intentionally coexists with staged, inert
    # response files until TASK-102-S5 installs the authoritative inventory.
    if "oracle_failures" in manifest:
        numbered_responses = {
            path.name
